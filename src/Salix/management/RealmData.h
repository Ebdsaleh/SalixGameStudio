...
namespace Salix {
    ...
    struct SALIX_API ReferencedAsset {
        ...
        // This struct represents the "manifest" or "header" for a realm file.
        // It contains lightweight metadata about the realm.

        struct RealmData {
            std::string realm_name;
            std::string realm_file_path_relative;  // Path to actual '.realm' file.
            ...
            template <class Archive>
            void serialize(Archive& archive) {
                archive(CEREAL_NVP( realm_name ),
                    CEREAL_NVP( realm_file_path_relative ),
                    CEREAL_NVP( entity_count ),
                    CEREAL_NVP( engine_version ),
                    CEREAL_NVP( unique_referenced_assets ));
            }
            RealmData() = default;  // Mandatory for Cereal deserialization
        };
