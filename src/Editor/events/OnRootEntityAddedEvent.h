// Editor/events/OnRootEntityAddedEvent.h
#pragma once
...
namespace Salix {
    // Dispatched AFTER a new ROOT entity has been added to the current_realm
    class OnRootEntityAddedEvent : public IEvent {
    public:
        OnRootEntityAddedEvent(EntityArchetype new_archetype)
            : archetype(std::move(new_archetype)) {}

        EntityArchetype archetype;

        EVENT_CLASS_TYPE(EditorOnRootEntityAdded)
        EVENT_CLASS_CATEGORY(EventCategory::Editor)
        CLONE_EVENT_METHOD(OnRootEntityAddedEvent)
    };
}